...
Window {
    ...
    Connections {
        target: viewModel
        function onRpmChanged(value) { tachometer.value = value; }
        function onGearChanged(value) { if(value === 0) gear.text = "N"; else gear.text = value }
    ...
    FastBlur {
        ...
        SequentialAnimation on radius { id: blurEnter;
            running: false
            onStarted: { blur.visible = true; }
            NumberAnimation { from: 0; to: 80; duration: 200 }
        }
        SequentialAnimation on radius { id: blurExit;
            running: false
    ...
    TaskSwitcher {
        ...
        SequentialAnimation on opacity { id: taskSwitcherEnter;
            running: false
            onStarted: { taskSwitcher.visible = true; }
            NumberAnimation { from: 0; to: 1; duration: 200 }
        }
        SequentialAnimation on opacity { id: taskSwitcherExit;
            running: false
